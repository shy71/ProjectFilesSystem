...
struct LabelTxtChunk
{
	...
	$assert(dwChunkSize>=24,"Hex Editor is unable to parse file: data size is less then 0");
...
struct FileChunk
{
	...
	$assert(dwChunkSize>=12,"Hex Editor is unable to parse file: data size is less then 0");
...
struct ListSubChunk
{
	...
	SizeSubChunks = SizeSubChunks - sizeof(chunkID) - sizeof(dwChunkSize) - dwChunkSize;
};
