	...
	if(Function)
	{
		...
		}else
		{
			if(Bound!=-1 && int(RvaToPa(Function))>0)
				ULONGLONG  AddressOfData as IMAGE_IMPORT_BY_NAME *(int(RvaToPaBiased(AddressOfData)));
		}
	}else
      $break_array(true);
};
...
struct IMAGE_IMPORT_DIRECTORY 
{
    ...
    union 
	{
		...
		if(Characteristics)
		{
			if(int(GetMachineType())==IMAGE_FILE_MACHINE_I386)
				DWORD   thunk as IMAGE_THUNK_DATA32ARRAY *(int(RvaToPaBiased(thunk)));
			else
				DWORD   thunk as IMAGE_THUNK_DATA64ARRAY *(int(RvaToPaBiased(thunk)));
		}
    } OriginalFirstThunk;

    TimeDateStamp   Bound;					// 0 if not bound,
	...
	union
	{
		...
		{
			if(int(GetMachineType())==IMAGE_FILE_MACHINE_I386)
				DWORD   ImportAddressTable as IMAGE_THUNK_DATA32ARRAY *(int(RvaToPaBiased(ImportAddressTable)));
			else
				DWORD   ImportAddressTable as IMAGE_THUNK_DATA64ARRAY *(int(RvaToPaBiased(ImportAddressTable)));
		}
	}FirstThunk;
...
public struct IMAGE_DEBUG_DIRECTORY
{
	...
	union 
	{
		DWORD	Rva;
		if(Rva)
			DWORD	Data as IMAGE_DEBUG_DIRECTORY_DATA* (int(RvaToPaBiased(Data)));
	}AddressOfRawData;
...
// Select a 32-bit or 64-bit header, depending on the FileHeader's Machine field
	...
	var _not_used_ = SetVirtualBase(OptionalHeader.ImageBase);

// Section directory	
	[noindex] IMAGE_SECTION_HEADER sections[FileHeader.NumberOfSections];

	// enumerate each data directory
	for(var i=0; i<OptionalHeader.NumberOfRvaAndSizes; i=i+1)
	{
		if(OptionalHeader.DataDirectory[i].VirtualAddress)
		{
			var va  = int(RvaToPa(OptionalHeader.DataDirectory[i].VirtualAddress));
			var tmp0 = SetDataDirectory(i,	OptionalHeader.DataDirectory[i].VirtualAddress,OptionalHeader.DataDirectory[i].Size);
...
// According to specification, there can be maximum of 96 sections
    ...
    ImageCharacteristicsType    Characteristics;
};
...
struct IMAGE_OPTIONAL_HEADER32
{
    ...
    DWORD   SizeOfStackReserve;		// widened in x64 (PE32+) version
    DWORD   SizeOfStackCommit;		// widened in x64 (PE32+) version
    DWORD   SizeOfHeapReserve;		// widened in x64 (PE32+) version
    DWORD   SizeOfHeapCommit;		// widened in x64 (PE32+) version
    DWORD   LoaderFlags;
...
public struct IMAGE_DOS_HEADER       // DOS .EXE header
{
	...
	WORD e_oeminfo;
	WORD e_res2[10];
	LONG e_lfanew as IMAGE_NT_HEADERS *(this);
};
