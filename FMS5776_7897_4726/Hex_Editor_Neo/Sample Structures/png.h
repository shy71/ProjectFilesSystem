...
{
	/*
	 val  | allowed bit depths		| description
...
hidden:
	unsigned __int64 SignatureAsInt;
};
...
struct ColorPoint
{
	DWORD X;
	DWORD Y;
	$print("pos(x,y)","{ "+double(X)/100000.+", "+double(Y)/100000.+" }");
};
...
struct Chunk
{
	...
	if(Length>=2000000)
	{
		...
		BYTE UnknownData[Length - (current_offset - nOffsetDataBegin)];			// Any non-described chunk

	DWORD CRC;		// Chunk cyclic redundance code
};
...
public struct PNG_FILE
{
	...
	$assert(Signature.SignatureAsInt==0x89504e470d0a1a0a);
